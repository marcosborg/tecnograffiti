                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.infos.update", [$info->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.info.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $info->name) }}" required>
                            @if($errors->has('name'))
